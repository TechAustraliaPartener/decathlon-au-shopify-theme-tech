...
const bindUI = () => {
  ...
  if (STATE.checkoutStep === CHECKOUT_STEPS.SHIPPING_METHOD) {
    if (STATE.deliveryMethod === 'ship') {
      document.getElementById(
        'checkout_shipping_rate_id_parcelify-pickup-0_00'
  ...
  if (STATE.checkoutStep === CHECKOUT_STEPS.PAYMENT_METHOD) {
    if (STATE.deliveryMethod === 'pickup') {
      document.querySelector('[data-different-billing-address]').style.display =
        'none';
      document.querySelector('[data-same-billing-address]').style.display =
        'none';
