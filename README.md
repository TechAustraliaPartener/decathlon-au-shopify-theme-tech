...
>Make it a habit to ==duplicate the published theme== and RECONCILE on from it first before starting any development activity
...
For Decathlon, we follow the **Createur's Git Branching model**. You may find the documentation [here](https://app.clickup.com/2702900/v/dc/2jfhm-12044/2jfhm-11545).

## The Repo

This repo has a lot going on. Here's what you need to know:

1. Most snippets are built from `snippets-src`. Each subfolder in that folder where the file is is prepended to the name. e.g. `snippets-src/assets/icons/available.liquid` becomes `snippets/assets-icons-available.liquid`.
Some snippets are just in `snippets` as usual. Check which one you're dealing with before working. If it's built, edit it in `snippets-src`. Built snippets are gitignored, so always look for the change you made in your commit to ensure it's there.
...
1. ==Do not touch== these directories and the files within it at all: `snippets , cypress , patterns `.
*Exception: Liquid snippets which are generated by apps (like algolia snippets), we can touch it directly within snippets directory.*
1. There is a RECONCILIATION.md file on this repository which contains the list of files that you need to watch out for when doing reconciliation. These files maybe updates from Shopify apps which we need to keep a copy of to prevent any issues in the future deployments.

## Major Mechanics

1. The the features on the cart relies on VueJS.
