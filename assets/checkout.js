!function() {
    ...
    var PROD_HOSTNAME = config.PROD_HOSTNAME;
    var ifDev = function(values) {
        return window && window.location && !window.location.hostname.match(PROD_HOSTNAME) ? values[0] : values[1];
    };
    var config$1 = {
        PICKUP_SHIPPING_METHODS: [ "Parcelify-decathlon%20san%20francisco%20store%20pickup-0.00" ],
        PICKUP_SHIPPING_METHOD: "Parcelify-decathlon%20san%20francisco%20store%20pickup-0.00",
        CLASSES: {
        ...
        STOREFRONT_API_KEY: ifDev([ "8e681070902104a65649736d6b1f7bd0", "f76765be5300c93b4689309b5a31e7eb" ])
    };
    var CLASSES = config$1.CLASSES, COPY = config$1.COPY;
    var updateUI = function() {
        ...
        showElements([ continueBtn, shippingAddressHeader, userAddressList ]), shippingAddressHeader.textContent = COPY.SHIPPING_ADDRESS_HEADING);
    };
    ...
    var CLASSES$1 = config$1.CLASSES, SHOP_ID = config$1.SHOP_ID, PICKUP_SHIPPING_METHOD = config$1.PICKUP_SHIPPING_METHOD, STOREFRONT_API_KEY = config$1.STOREFRONT_API_KEY;
    ...
    var PICKUP_SHIPPING_METHOD$1 = config$1.PICKUP_SHIPPING_METHOD;
    var CUSTOM_UI_SELECTORS$1 = {
        PICKUP_SHIPPING_METHOD_BLOCKS: config$1.PICKUP_SHIPPING_METHODS.map(function(selector) {
            return '[data-shipping-method="' + selector + '"]';
        }),
        PICKUP_SHIPPING_METHOD: '[data-shipping-method="' + PICKUP_SHIPPING_METHOD$1 + '"]',
        LOADING_OVERLAY: ".de-loading-overlay",
        LOADING_IMAGE: ".de-checkout-loader"
    };
    ...
    document.addEventListener("page:load", function() {
        ...
        STATE.checkoutStep === CHECKOUT_STEPS.CONTACT_INFORMATION && function() {
            ...
            pickupToggleBtn.addEventListener("click", function(event) {
                ...
                updateUI();
            }), null !== STATE.pickupStore && (mapImage.src = window[STATE.pickupStore]);
                            ...
                            fetch("/api/graphql", {
                                ...
                                console.log(data), function(checkoutGID, checkoutKey) {
                                    fetch("/api/graphql", {
                                        ...
                                        var checkoutURL = "/" + SHOP_ID + "/checkouts/" + window.Shopify.Checkout.token + "?key=" + checkoutKey;
                ...
                document.querySelectorAll(SELECTORS.PICKUP_LOCATION).forEach(function(location) {
                    location.addEventListener("click", function(e) {
                        if (!this.classList.contains(CLASSES$1.ACTIVE_PICKUP_LOCATION)) {
                            ...
                            console.log(pickupStore), mapImage.src = window[pickupStore];
                        }
                    });
