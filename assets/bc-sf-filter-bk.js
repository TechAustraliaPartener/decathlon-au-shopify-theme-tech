...
BCSfFilter.prototype.buildProductGridItem = function(data) {
    ...
    var colorChips = [];
    var colorImageMap = {};
    data.options_with_values.forEach(function(option) {
    ...
    data.variants.forEach(function(variant, index) {
      if (variant.hasOwnProperty('option_color')) {
        var colorExists = false;
        for (var i = 0; i < colorChips.length; i++) {
          if (colorChips[i].name == variant.option_color) {
            colorExists = true;
            break;
          }
        }
        if (!colorExists && Object.keys(colorImageMap).length > 0) {
          var variantImage = colorImageMap[variant.option_color];
          if (variantImage) {
            variantImage = variantImage.replace(/\.\w{3,4}($|\?v=)/g, '_600x$&');
          }

          var temp = {
            name: variant.option_color,
            image: variantImage,
    ...
    temp.product.options.forEach(function(option, index) {
      ...
      temp.product.variants.forEach(function(variant) {
        variant['option'+ (index+1)] = variant['option_'+ name];
      });
    });
...
BCSfFilter.prototype.buildPagination = function(totalProduct) {
    ...
    if (totalPage == 1) {
        jQ(this.bottomPaginationSelector).html('');
        return false;
    }
    ...
    if (this.getSettingValue('general.paginationType') == 'default') {
        ...
        jQ(this.bottomPaginationSelector).html(paginationHtml);
    }
};
